...
namespace ved {
	...
	void
		copy_range(HANDLE h_from,
			HANDLE h_to,
			const LARGE_INTEGER i_pos,
			LARGE_INTEGER iSize)
	{
		...
		while (iSize.QuadPart > 0) {
			if (!::ReadFile(h_from,
				buf.data(),
				static_cast<DWORD>(min(buf.size(), iSize.QuadPart)),
				&dw_bytes_read,
				nullptr))
				throw ved::sparse_exception(L"Error ReadFile!", ::GetLastError());

			if (!::WriteFile(
				h_to, buf.data(), dw_bytes_read, &dw_bytes_written, nullptr))
	...
	void
		copy_sparse::copy(const std::wstring& in, const std::wstring& out)
	{
		...
		if (!sz_drive.empty()) {
			...
			if (!::GetVolumeInformation(sz_drive.c_str(),
				sz_vol_name.data(),
				MAX_PATH,
				&dw_sn,
				&dw_max_len,
				...
				sz_file_name.data(),
				_MAX_FNAME))
				throw ved::sparse_exception(L"Error GetVolumeInformation!",
					::GetLastError());
		...
		if (!::GetFileInformationByHandle(h_in_file.operator*(), &bhfi))
			throw ved::sparse_exception(L"Error GetFileInformationByHandle!",
				::GetLastError());

		b_sparse_copy =
			b_sparse_copy &&
			(bhfi.dwFileAttributes &
				FILE_ATTRIBUTE_SPARSE_FILE); // Check if the source file is sparse
		...
		settings.dw_flags_and_attributes =
		...
		if (b_sparse_copy) {
			...
			i_file_size.LowPart = ::GetFileSize(
				h_in_file.operator*(), reinterpret_cast<LPDWORD>(&i_file_size.HighPart));
			if (i_file_size.LowPart == static_cast<DWORD>(-1)) {
				throw ved::sparse_exception(L"Error GetFileSize!", ::GetLastError());
			}

			DWORD dw_temp;
			if (!::DeviceIoControl(h_out_file.operator*(),
				FSCTL_SET_SPARSE,
				nullptr,
				0,
				nullptr,
				...
				&dw_temp,
				nullptr))
				throw ved::sparse_exception(L"Error DeviceIoControl!(FSCTL_SET_SPARSE)",
					::GetLastError());
			...
			do { // This loop executes more than once only if the file has more than
			  // 1024 allocated areas
				br = ::DeviceIoControl(h_in_file.operator*(),
				...
				if (!br) {
					...
					if (dr != ERROR_MORE_DATA)
						throw ved::sparse_exception(
							L"Error DeviceIoControl!(FSCTL_QUERY_ALLOCATED_RANGES)",
							::GetLastError());
				}

				n = nbytes / sizeof(FILE_ALLOCATED_RANGE_BUFFER); // Calculate the number
																  // of records returned
				for (i = 0; i < n; i++)                           // Main loop
					copy_range(h_in_file.operator*(),
						h_out_file.operator*(),
						ranges[i].FileOffset,
						ranges[i].Length);
				...
				if (!br && n > 0) {
					queryrange.FileOffset.QuadPart =
						ranges[n - 1].FileOffset.QuadPart + ranges[n - 1].Length.QuadPart;
					queryrange.Length.QuadPart =
						i_file_size.QuadPart - queryrange.FileOffset.QuadPart;
				}
			} while (!br); // Continue loop if ERROR_MORE_DATA

			// Set end of file (required because there can be a sparse area in the end)
			dr = ::SetFilePointer(h_out_file.operator*(),
				i_file_size.LowPart,
				&i_file_size.HighPart,
				FILE_BEGIN);
			...
			do {
				...
				if (dw_bytes_read) {
					...
					if (dw_bytes_written < dw_bytes_read)
						throw ved::sparse_exception(L"Error ERROR_HANDLE_DISK_FULL!",
							ERROR_HANDLE_DISK_FULL);
				}
			} while (!buf.empty());
		}

		if (!::SetFileTime(h_out_file.operator*(),
			&bhfi.ftCreationTime,
			&bhfi.ftLastAccessTime,
			&bhfi.ftLastWriteTime))
			throw ved::sparse_exception(L"Error SetFileTime!", ::GetLastError());

		if (!::SetFileAttributes(out.c_str(),
			bhfi.dwFileAttributes & ALLOWED_ATTRIBUTES))
