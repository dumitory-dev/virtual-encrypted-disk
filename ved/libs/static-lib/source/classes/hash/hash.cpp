...
namespace ved {
	...
	std::string
		hash::get_hash(const BYTE* data, const DWORD size) const
	{
		HCRYPTPROV h_prov{};
		HCRYPTPROV h_hash{};

		if (!::CryptAcquireContext(
			&h_prov, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
		...
		if (!::CryptCreateHash(
			h_prov, static_cast<unsigned>(this->type_), 0, 0, &h_hash)) {
		...
		if (!::CryptHashData(h_hash, data, size, 0)) {
