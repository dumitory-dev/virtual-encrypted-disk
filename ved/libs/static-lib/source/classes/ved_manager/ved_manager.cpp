...
namespace ved {
	void
		ved_manager::mount(const std::wstring& path,
			...
			WCHAR letter)
	{
		...
		LARGE_INTEGER size;
		size.QuadPart = 0;
		const auto disk_info = ved::driver_disk::make_file_info(
			path, size, letter, hash_password, Crypt::CryptPrev);
		this->driver_.mount_disk(disk_info);
	}

	void
		ved_manager::mount_ex(const std::wstring& path,
			...
			const Crypt mode)
	{
		...
		const auto disk_info = ved::driver_disk::make_file_info(
			path, size_file, letter, hash_password, mode);
		this->driver_.mount_disk(disk_info);
	}

	void
		ved_manager::create_file(const std::wstring& path,
			const unsigned long long size,
			const std::string& password,
			const Crypt mode)
	{
		...
		size_file.QuadPart = size;
		const auto disk_info = ved::driver_disk::make_file_info(
			path, size_file, L'C', hash_password, mode);
		this->driver_.create_file_disk(disk_info);
	}

	void
		ved_manager::un_mount(const WCHAR letter)
	{
		this->connected();
	...
	void
		ved_manager::run_driver(DWORD flag_startup,
			const std::wstring& path_driver,
			const std::wstring& name_service)
	{

		ved::install_manager::install_service(
			path_driver, name_service, flag_startup);
		ved::install_manager::load_service(name_service);
	}

	void
		ved_manager::stop_driver(const std::wstring& device_name)
	{
	...
	std::vector<OPEN_FILE_INFORMATION>
		ved_manager::get_mounted_disks(void)
	{
		this->connected();
	...
	std::string
		ved_manager::make_md5_hash(const std::string& password) const
	{
		auto hash =
			this->hash_convert_.get_hash({ password.cbegin(), password.cend() });
