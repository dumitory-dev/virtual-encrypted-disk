...
namespace ved {
	...
	public:
		ved_manager(void)
			: driver_(MAIN_DEVICE_SYM_LINK)
		{}

		~ved_manager(void) override = default;
		...
		void mount(const std::wstring& path,
			const std::string& password,
			WCHAR letter) override;

		void mount_ex(const std::wstring& path,
			unsigned long long size,
			const std::string& password,
			WCHAR letter,
			Crypt mode) override;

		void create_file(const std::wstring& path,
			unsigned long long size,
			const std::string& password,
			Crypt mode) override;
		...
		void run_driver(DWORD flag_startup = SERVICE_DEMAND_START,
			const std::wstring& path_driver =
			L"%SystemRoot%\\System32\\Drivers\\DriverVED.sys",
			const std::wstring& name_service = L"VEDriver") override;
