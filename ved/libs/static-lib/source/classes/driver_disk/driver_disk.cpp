...
#include "constant.h"
#include "classes/define_device/define_device_manager.h"
namespace ved {
	std::unique_ptr<OPEN_FILE_INFORMATION>
		driver_disk::make_file_info(const std::wstring& file_name,
			...
			enum Crypt mode_crypt)
	{
		...
		std::unique_ptr<OPEN_FILE_INFORMATION> uptr_file_info =
			std::make_unique<OPEN_FILE_INFORMATION>();

		wcscpy_s(uptr_file_info->FileName, L"\\??\\");
		wcscat_s(uptr_file_info->FileName, file_name.c_str());

		uptr_file_info->FileNameLength =
			static_cast<USHORT>(wcslen(uptr_file_info->FileName));
		uptr_file_info->FileSize.QuadPart = file_size.QuadPart;
		uptr_file_info->DriveLetter = drive_letter;

		std::copy(
			file_password.cbegin(), file_password.cend(), uptr_file_info->Password);
			...
			const std::unique_ptr<OPEN_FILE_INFORMATION>& open_file) const
	{
		...
		const auto path_new_device =
			DEVICE_NAME_PREFIX + std::to_wstring(number_device);
		...
		try {
			...
			define_device.send_ctl_code(IOCTL_FILE_DISK_OPEN_FILE,
				open_file.get(),
				sizeof(_OPEN_FILE_INFORMATION) +
				open_file->FileNameLength * sizeof(WCHAR) +
				7);
	...
	void
		driver_disk::create_file_disk(
			const std::unique_ptr<OPEN_FILE_INFORMATION>& open_file) const
	{
		if (const auto ret = this->main_device_.send_ctl_code(
			IOCTL_FILE_DISK_OPEN_FILE,
			open_file.get(),
			sizeof(_OPEN_FILE_INFORMATION) + open_file->FileNameLength * sizeof(WCHAR) +
			7); ret != 0) {
	...
	size_t
		driver_disk::get_free_number_device(void) const
	{
		size_t number_device =
			this->main_device_.send_ctl_code(IOCTL_GET_FREE_DEVICE);
	...
	void
		driver_disk::un_mount_disk(const WCHAR letter)
	{
	...
	void
		driver_disk::copy_image(const std::wstring& in, const std::wstring& out)
	{
		ved::copy_sparse::copy(in, out);
	}

	std::vector<OPEN_FILE_INFORMATION>
		driver_disk::get_mounted_disks(void) const
	{

		const size_t max_count_disk = 25;
		std::vector<OPEN_FILE_INFORMATION> data(max_count_disk,
			OPEN_FILE_INFORMATION());
		const auto ret = this->main_device_.send_ctl_code(
			IOCTL_GET_MOUNT_DEVICES,
			nullptr,
			0,
			&data[0],
			max_count_disk * sizeof(OPEN_FILE_INFORMATION));
		data.resize(ret);
