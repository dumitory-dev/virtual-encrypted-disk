...
namespace ved {
		...
		engine::execute(void)
	{
		...
		if (name_func != L"-start" && name_func != L"-stop" &&
			...
			if (this->arguments_stream_.eof()) {
				throw std::runtime_error(
					"Invalid argument! Please, run the command -help");
			}
		}
		...
		if (res_func == this->functions_.cend()) {
			throw std::runtime_error(
				"Invalid arguments! Please, run the command -help");
		}
	...
	void
		engine::set_arguments(void)
	{

		this->arguments_stream_ =
			ved::command_line_argument::get_command_lines_stream();
		...
		engine::mount(void)
	{
		...
		if (!(this->arguments_stream_ >> path) ||
		...
		engine::create_and_mount()
	{
		...
		this->ved_manager_->mount_ex(path,
			size,
		...
		engine::create_file()
	{
		...
		this->ved_manager_->create_file(path,
			size,
			{ password.cbegin(), password.cend() },
			static_cast<Crypt>(mode));
	}
		...
		engine::start_driver(void)
	{
		...
		if (!startup_flag.empty() && startup_flag != L"-auto") {
			throw std::runtime_error(
				"invalid argument!(un_mount)  Please, run the command -help");
		}
		...
		engine::help(void) const
	{
		...
		std::wcout << L"\n-camount - create file and mount disk. Params:\n1.Path "
			L"file\n2.Password\n3.Size file(mb)\n4.Letter disk\n5.Mode "
			L"crypt(1 - RC4, 2 - AES)\n\n";
		std::wcout << L"\n-unmount - Unmount disk. Params:\n1.Letter disk or -all "
			L"for unmount all disks\n\n";
		std::wcout << L"\n-show - Show mounted disks. Params:\n none\n";
		std::wcout
			<< L"\n-copy - Copy image disk. Params:\n1. Source path\n2.Dest path\n";
		std::wcout
			<< L"\n-start - Install and start driver. Params:\n1.Mode(optional) (-auto "
			L"- autostart driver at system startup)\n";
	}

	void
		engine::show_mounted_disks(void) const
	{
		...
		for (const auto& open_file_information : data) {
			std::wcout << "\n"
				<< "Path file - " << std::wstring(open_file_information.FileName)
				<< std::endl;
			std::wcout << "Letter disk - " << open_file_information.DriveLetter
				<< std::endl;
			std::wcout << "Size file(mb) - "
				<< open_file_information.FileSize.QuadPart / 1024 / 1024
				<< std::endl;
			std::wcout << "Crypt mode  - " << open_file_information.CryptMode
				<< std::endl;
		}
	}
